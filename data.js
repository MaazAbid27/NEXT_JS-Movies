...
const movies = [
  ...
  export function GetGenresById(id){
    return genres.find((m)=>m.id===id)
  }
  export function GetMoviesByGenreId(id){
    return movies.filter((m)=>m.genreId===id)
  }

  export function GetDirectorByMovieId(id){
    const director = directors.find(d => d.id === movies.directorId);
    return director
  }
